// Compile: g++ -std=c++23 -O2 -W  -lstdc++exp -o simple-stacktrace-iteration
// simple-stacktrace-iteration.cpp
#include <iostream>
#include <print>
#include <stacktrace>
#include <string_view>

void print_current_stacktrace_flat(std::stacktrace trace = std::stacktrace::current())
{
    ...
    for (std::size_t i = trace.size() - 1; i < trace.size(); --i)
    {
        ...
        {
            std::println("  Location: {}:{}", entry.source_file(), entry.source_line());
        }
